...
from PySide6.QtWidgets import (
    QMainWindow, QWidget, QTabWidget, QVBoxLayout, QHBoxLayout, QToolBar, QMenuBar,
...
<ul>
  ...
  <li>여러 파일 동시 업로드 가능 (배치 업로드 지원)</li>
  <li>업로드 진행 중 '❌ 취소' 버튼으로 중단 가능</li>
  <li>업로드된 문서는 자동으로 임베딩되어 검색 가능</li>
  <li>상세한 진행 상황 표시 (파일 저장 → 분석 → 청크 생성 → 임베딩)</li>
</ul>

<p><b>2. 질문하기</b></p>
<ul>
  <li>하단 입력창에 질문 작성 (Shift+Enter로 줄바꿈)</li>
...
<p><b>3. 검색 범위 선택</b></p>
<ul>
  <li>대화창 상단에서 검색 범위 선택 가능</li>
  <li>통합: 개인 + 공유 DB 모두 검색</li>
  <li>개인: 내 컴퓨터의 문서만 검색</li>
  <li>공유: 공용 DB의 문서만 검색 (공용 DB 설정 필요)</li>
</ul>

<p><b>4. 파일 멘션 기능</b></p>
<ul>
  <li>질문 입력 시 '@' 기호를 입력하면 파일 목록이 자동으로 표시됨</li>
  <li>Tab/Enter로 파일 선택, Esc로 취소</li>
...
<ul>
  ...
  <li>'➕ 새 대화' 버튼으로 새 세션 시작</li>
  <li>더블클릭으로 대화 불러오기</li>
  <li>'내보내기'로 대화 저장</li>
  <li>'◀ 접기' 버튼으로 사이드바 접기/펼치기</li>
</ul>

<p><b>6. 설정</b></p>
<ul>
  <li>'설정' 탭에서 LLM, 임베딩, Vision API 설정</li>
  <li>LLM 설정: 답변 생성용 모델 설정</li>
  <li>임베딩 설정: 문서 검색용 모델 설정</li>
  <li>Vision API 설정: PDF/PPTX Vision 분석용 설정</li>
  <li>공용 DB: 네트워크 드라이브의 공유 문서 DB 경로 설정</li>
  <li>설정 변경 후 '설정 저장' 클릭</li>
  <li>공용 DB 연결 문제 시: 메뉴 → 설정 → 공용DB 재접속</li>
</ul>
...
<ul>
  ...
  <li>네트워크 드라이브 재연결 후 '공용DB 재접속' 메뉴 사용</li>
  <li>사이드바가 좁으면 '접기' 버튼으로 대화 화면 확장</li>
</ul>
        """
