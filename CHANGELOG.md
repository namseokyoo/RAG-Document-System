...
### 🔍 성능 모니터링 및 분석
- **타이밍 헬퍼 추가**: RAGChain에 단계별 시간 측정 기능 구현
  - 분류, 멀티쿼리 생성, 원본 검색, 분해 병렬 검색, 재랭크, 컨텍스트 총합, 검증/재생성, Citation 등 주요 단계별 시간 누적
  - 반환 딕셔너리에 `timings` 포함하여 병목 식별 지원
  - 파이프라인 병목 후보 분석 및 최적화 포인트 도출

### 🐛 버그 수정
- **파일 멘션 질문 캐시 문제 해결**: 새 대화 시작 시 이전 질문이 섞이는 문제 수정
  - `_original_question` 갱신 로직 개선
  - `RAGChain.clear_memory()` 구현하여 새 대화 시작 시 캐시·세션 초기화
  - `MainWindow._start_new_chat`에서 메모리 초기화 호출 연결

### 📝 프롬프트 개선
- **프롬프트 한글화**: 기본 프롬프트 전면 한글화
  - 영어 관용구(According to 등) 사용 금지
  - 출처 표기를 한국어 문장형으로 안내하는 예시 추가
  - 첫 문장이 영어로 시작되는 문제 차단

### 🎨 UI/UX 개선
- **채팅 화면 스크롤 개선**: 더 부드럽고 작은 단위로 이동
  - 스크롤바 `singleStep`: 20 → 5
  - 스크롤바 `pageStep`: 80 → 30
  - 애니메이션 duration: 100ms → 150ms
  - Easing curve: `OutQuad` → `OutCubic`

- **실시간 처리 단계 표시**: 질문 후 실제 처리 단계 추적 및 표시
  - QTimer를 사용하여 200ms마다 RAGChain의 실제 처리 단계 모니터링
  - 단계별 메시지: "🎯 질문 유형 분석 중", "🌐 질문 번역 중", "🔄 다중 쿼리 생성 중", "💡 가상 문서 생성 중", "📚 문서 검색 중", "🔄 하위 질문 검색 중", "⭐ 문서 재랭킹 중", "💬 응답 생성 중"
  - "..." 애니메이션: 점 개수가 0~3개로 반복 (500ms마다)
  - 첫 청크 도착 시 애니메이션 자동 중지

### 🧪 질문 분류기 개선
- **정확도 향상**: 전체 정확도 96.00% 달성
  - `_apply_keyword_boosting` 중복 부스팅 방지 (positive는 카테고리당 1회만 가산)
  - 리스트/식별자 부재 시 `exhaustive_list` 보정
  - 검색어 기반 simple_keyword vs fact 경계 보정

### 📊 성능 분석
- **파이프라인 병목 분석**: 질문→응답 파이프라인 전체 흐름 분석
  - 번역/분류/의도감지→검색전략→쿼리분해→멀티쿼리→재랭크→검증/재생성 흐름 확인
  - 키워드 질문 시 HyDE·다중쿼리·재랭크 생략 분기 활용
  - Self-Consistency, 검증/재생성, Re-ranker, Vision 청킹 등 상황별 최적화 포인트 기록
