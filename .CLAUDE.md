...
### 🔧 Phase 3.5: 대규모 DB 신규 문서 우선순위 개선 (진행 중)
**목표**: 963개 문서 환경에서도 신규 업로드 PDF 검색 보장
**우선순위**: **Critical** (사용자 경험 직결)
**예상 소요 시간**: 2-3일

#### 배경
**문제**: 격리 환경(63개)에서는 완벽 작동, 프로덕션(963개)에서는 신규 문서 검색 안 됨

**테스트 결과**:
| 환경 | 문서 수 | 신규 PDF 검색 | RAG 소스 사용 |
|------|---------|---------------|---------------|
| 격리 (test_chroma_db) | 63 | ✅ 상위 3개 | ✅ 3개 페이지 |
| 프로덕션 (chroma_db) | 963 | ✗ Top-k에 없음 | ✗ 다른 문서 사용 |

#### 해결 방안 (3가지 옵션)

**Option A: 시간 기반 부스팅** (추천)
```python
# 최근 업로드 문서 점수 부스팅
def boost_recent_documents(results, days_threshold=7):
    for result in results:
        upload_date = result.metadata.get('upload_timestamp')
        if (now - upload_date).days < days_threshold:
            result.score *= 1.5  # 50% 점수 부스트
...
**Option B: 문서별 검색 필터** (중기 솔루션)
```python
# GUI에서 문서 선택 후 해당 문서만 검색
def search_with_filter(query, document_ids=None):
    if document_ids:
        # 특정 문서들만 검색
...
- GUI 변경 필요 (문서 선택 UI)
- 사용자가 매번 문서 선택해야 함
- 다른 관련 문서 발견 못 할 수 있음

**예상 소요 시간**: 2-3일 (UI 개발 포함)

---

**Option C: 개인 DB 우선 검색** (장기 솔루션)
```python
# 개인 DB 우선 검색 → 없으면 공유 DB
def hybrid_db_search(query, personal_db, shared_db):
    # 1. 개인 DB에서 먼저 검색 (top_k=5)
    personal_results = personal_db.similarity_search(query, k=5)

    if len(personal_results) >= 3:
        # 개인 DB에서 충분히 찾음
        return personal_results
    else:
        # 개인 DB 부족 → 공유 DB에서 보충
        shared_results = shared_db.similarity_search(query, k=10)
        return combine_and_rerank(personal_results, shared_results)
```

**장점**:
- 개인 업로드 문서 항상 우선
- 공유 DB도 활용 가능 (best of both worlds)
- 확장성 좋음 (사용자별 DB 분리)
...
#### 3.5.1 즉시 구현 계획 (Option A)

**Step 1: upload_timestamp 메타데이터 추가** (30분)
```python
...
**Step 2: 시간 기반 부스팅 함수 구현** (1-2시간)
```python
# utils/rag_chain.py
def boost_recent_documents(self, results, boost_factor=1.5, days=7):
    now = datetime.now()
    for result in results:
        upload_time = result.metadata.get('upload_timestamp')
        if upload_time:
            upload_date = datetime.fromisoformat(upload_time)
            if (now - upload_date).days < days:
                result.score *= boost_factor
    return sorted(results, key=lambda r: r.score, reverse=True)
```

**Step 3: RAG Chain 통합** (1-2시간)
```python
# Retrieval 후 즉시 부스팅 적용
retrieved_docs = self.retriever.get_relevant_documents(query)
boosted_docs = self.boost_recent_documents(retrieved_docs)
context = self.create_context(boosted_docs)
```

**Step 4: 프로덕션 환경 테스트** (2-3시간)
- 963개 문서 DB에서 신규 PDF 업로드
- 검색 우선순위 확인
- RAG 쿼리 소스 검증

**Step 5: 파라미터 튜닝** (1-2시간)
- boost_factor: 1.3, 1.5, 2.0 비교
- days_threshold: 3일, 7일, 14일 비교
- 최적 조합 선정
...
#### 3.5.3 장기 구현 계획 (Option C - Phase 4)

개인 DB / 공유 DB 완전 분리:
- [ ] VectorStoreManager 이중 DB 지원
- [ ] 우선순위 검색 로직 구현
- [ ] DB 마이그레이션 도구
- [ ] 성능 최적화 (병렬 검색)
